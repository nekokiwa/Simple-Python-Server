...
def test_download(server, client):
    os.remove(DOWNLOADS_FOLDER + 'hello.txt')



def setup_test():
    """setup and begin running the server for testing"""
    #setup
    client_names:list[ClientName] = []
    connected_clients:list[ClientHolder] = []
    load_names(client_names, TEST_NAME_FILE)
    old_client_names = client_names

    port:int = PORT_TO_USE
    s:SocketType = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    host:str = socket.gethostname()
    
    log_message(TEST_LOG_FILE,f'\n##########################\nstarting new session with:\nip:{host}||port:{port}')
    s.bind((host,port))
    s.listen(100)
    running:bool = True
    ...
    def on_exit():
        #log end of session
        log_message(TEST_LOG_FILE,f"ending test session with:\nip:{host}||port:{port}")
    ...
    while running:
        try:
            ...
            log_message(TEST_LOG_FILE,f"exception: ({x}) happened in main code...")
            pass
